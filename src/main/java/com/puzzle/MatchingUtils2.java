...
public class MatchingUtils2 {
    ...
    public static List<Piece> getAllBothTR_BR(Map<Piece, List<Piece>> pieces) {

        return pieces.keySet().stream().filter(p->p.getLeft()==0 && p.getTop()==0 && p.getBottom()==0).collect(Collectors.toList());
